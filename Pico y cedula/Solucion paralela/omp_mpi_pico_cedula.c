...
void main(int argc, char *argv[]){
    ...
    int n_cedulas=strtol(argv[2], NULL, 10);
...
//-----------------------contar cedulas pares e impares y los digitos en estas-----------------------------------
    ...
    char digito[1]; 
    int  p=0,ultimo_digito,j,i,k,*aux;

    #pragma omp parallel for shared(i,n_cedulas,num_cedulas_par,num_cedulas_impar,num_digitos_par,num_digitos_impar)   private(digito,p,ultimo_digito,aux)
    for(i=1;i<=n_cedulas;i++){
          fgets(digito, 2, fileToread);
          //-------------------jugar con digito[0]-------------------------------------------------------
          while(digito[0]!=10){
